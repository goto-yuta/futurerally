/**
 * Sync all Japanese WTA-ranked players from Sackmann CSV + Wikidata.
 *
 * Steps:
 *  1. Fetch wta_rankings_current.csv and wta_players.csv from Sackmann's GitHub
 ...
 *     NOTE: atp_player_id column is reused for WTA player IDs (rename deferred).
 *           WTA IDs are in the 200000+ range and do not overlap with ATP IDs.
 *  5. Print stats: inserted N, updated N, Wikidata-verified N/total
 *
 * Run:
 *   npm run rankings:sync:wta
 */
...
async function main() {
  console.log("Fetching Sackmann WTA CSVs...");
  ...
  console.log(`Latest WTA ranking date: ${latestDate}`);
  ...
  console.log(`Japanese WTA-ranked players found: ${jpnRankings.length}`);
  ...
  for (const rankRow of jpnRankings) {
    ...
    // slug: wta- prefix to avoid collision with ATP slugs of same name
    const slug = `wta-${slugify(`${p.nameLast} ${p.nameFirst}`)}`;
    ...
    const payload = {
      ...
      // atpPlayerId column reused for WTA IDs (rename deferred)
    ...
    try {
      ...
      if (result.length > 0) {
        inserted++;
        console.log(`  [UPSERT] rank#${rankRow.rank} ${nameEn} (${nameJa}) wta_id=${p.playerId}`);
      }
    } catch (err) {
      // Slug conflict: append wta_player_id to disambiguate
      const fallbackSlug = `wta-${slugify(`${p.nameLast} ${p.nameFirst}`)}-${p.playerId}`;
      try {
        await db
          .insert(players)
        ...
        console.error(`  [ERROR] rank#${rankRow.rank} ${nameEn} wta_id=${p.playerId}:`, err2);
      }
    }
  }

  console.log("");
  console.log("=== WTA Sync complete ===");
